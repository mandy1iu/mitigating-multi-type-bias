...
def _load_gender_data(persistent_dir):
    ...
    # We collect 10000 of each class of sentences. change to 1000
    n_sentences = 3000
    count_male_sentences = 0
    count_female_sentences = 0
    count_neutral_sentences = 0

    with open(f"{persistent_dir}/data/text/wikipedia-9.txt", "r") as f:
        lines = f.readlines()
...
def _load_race_data(persistent_dir):
    ...
    with open(f"{persistent_dir}/data/text/wikipedia-2.txt", "r") as f:
        lines = f.readlines()
    random.shuffle(lines)

...
def _split_gender_dataset(male_feat, female_feat, neut_feat):
    ...
    # change label method 1
    y_male = np.ones(male_feat.shape[0], dtype=int)
    y_female = np.ones(female_feat.shape[0], dtype=int)
...
def _apply_nullspace_projection(
    X_train, X_dev, X_test, Y_train, Y_dev, Y_test, n_classifiers=30
):
...
def before_compute_projection_matrix(model, tokenizer, data, bias_type):
    if bias_type == "gender":
...
def compute_projection_matrix(X_train, X_dev, X_test, Y_train, Y_dev, Y_test, n_classifiers=80):
    """Runs INLP.

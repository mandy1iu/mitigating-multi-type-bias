...
def main():
    ...
    if data_args.counterfactual_augmentation is not None:
        ...
        #     "./data/bias_attribute_words.json",
        #     bias_type=data_args.counterfactual_augmentation,
        # )

        bias_attribute_words_gender = _create_bias_attribute_words(
            "./data/bias_attribute_words.json",
            bias_type="gender",
        )
        bias_attribute_words_race = _create_bias_attribute_words(
            "./data/bias_attribute_words.json",
            bias_type="race",
        )
        bias_attribute_words_religion = _create_bias_attribute_words(
            "./data/bias_attribute_words.json",
            bias_type="religion",
        )

        counterfactual_augmentation_gender_func = partial(
            gender_counterfactual_augmentation,
            bias_attribute_words=bias_attribute_words_gender,
        )
        counterfactual_augmentation_race_func = partial(
            ternary_counterfactual_augmentation,
            bias_attribute_words=bias_attribute_words_race,
        )
        counterfactual_augmentation_religion_func = partial(
            ternary_counterfactual_augmentation,
            bias_attribute_words=bias_attribute_words_religion,
        )


        tokenized_gender_datasets = tokenized_datasets.map(
            counterfactual_augmentation_gender_func,
            batched=True,
            num_proc=data_args.preprocessing_num_workers,
            load_from_cache_file=not data_args.overwrite_cache,
            desc=f"Applying counterfactual augmentation for gender",
        )
        tokenized_race_datasets = tokenized_datasets.map(
            counterfactual_augmentation_race_func,
            batched=True,
            num_proc=data_args.preprocessing_num_workers,
            load_from_cache_file=not data_args.overwrite_cache,
            desc=f"Applying counterfactual augmentation for race",
        )
        tokenized_religion_datasets = tokenized_datasets.map(
            counterfactual_augmentation_religion_func,
            batched=True,
            num_proc=data_args.preprocessing_num_workers,
            load_from_cache_file=not data_args.overwrite_cache,
            desc=f"Applying counterfactual augmentation for religion",
        )
        print(type(tokenized_gender_datasets))
        print(type(tokenized_race_datasets))
        print(type(tokenized_religion_datasets))

        # tokenized_datasets = concatenate_datasets([tokenized_gender_datasets,tokenized_race_datasets,tokenized_religion_datasets])
        combined_train_datasets = concatenate_datasets([
          # tokenized_gender_datasets["train"],
          tokenized_race_datasets["train"],
          tokenized_religion_datasets["train"]
        ])

        combined_validation_datasets = concatenate_datasets([
          # tokenized_gender_datasets["validation"],
          tokenized_race_datasets["validation"],
          tokenized_religion_datasets["validation"]
        ])


        # 将 Dataset 转换为列表
        combined_train_list = combined_train_datasets.to_pandas().values.tolist()
        combined_validation_list = combined_validation_datasets.to_pandas().values.tolist()

        # 打乱训练数据集
        random.shuffle(combined_train_list)

        # 打乱验证数据集
        random.shuffle(combined_validation_list)

        # 将列表重新转换为 Dataset 对象
        combined_train_datasets = Dataset.from_pandas(pd.DataFrame(combined_train_list, columns=combined_train_datasets.column_names))
        combined_validation_datasets = Dataset.from_pandas(pd.DataFrame(combined_validation_list, columns=combined_validation_datasets.column_names))


        tokenized_datasets = DatasetDict({
          "train": combined_train_datasets,
          "validation": combined_validation_datasets
        })
